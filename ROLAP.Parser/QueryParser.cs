...
using ROLAP.Common.Model.QueryFunctions;
...
public static class QueryParser
{
    ...
    public static CubeQuery Parse(string input)
    {
        _lexer.Parse(input);
        _lexer.Print();
    ...
    private static CubeQuery GetSelectQuery()
    {
        ThrowIfNextTokenFailed();

        List<CubeAxisQuery> axes = new List<CubeAxisQuery>
        {
            AxisQuery()
        };
        ...
        return new CubeQuery(QueryType.SELECT, axes, GetCubeName());
    }

    private static CubeAxisQuery AxisQuery()
    {
        var member = SetQuery();
        ...
        return new CubeAxisQuery(member, number);
    }

    private static ICubeQueryItem SetQuery()
    {
    ...
    private static ICubeQueryItem GetSet()
    {
        ThrowIfTokenTypeNotValid(TokenType.LBrace);
        ThrowIfNextTokenFailed();
        
        List<ICubeQueryItem> res = new List<ICubeQueryItem>()
        {
            SetQuery()
        };
        ...
        return new CubeSetQuery(res);
    }

    private static ICubeQueryItem GetTuple()
    {
        ThrowIfTokenTypeNotValid(TokenType.LParent);
        ThrowIfNextTokenFailed();
        
        var items = new List<ICubeQueryItem>
        {
            GetMember()
        };
    ...
    private static ICubeQueryItem GetFunction()
    {
        string name;
        ...
        List<ICubeQueryItem> args = new List<ICubeQueryItem>
        {
            SetQuery()
        };
